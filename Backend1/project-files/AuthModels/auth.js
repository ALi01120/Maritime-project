...
router.put('/profile', verifyToken, upload.single('image'), async (req, res) => {
  ...
  try {
    // Fetch current user data to preserve existing profile_image if no new image is uploaded
    const selectSql = 'SELECT profile_image FROM users WHERE id = ?';
    db.query(selectSql, [userId], (err, results) => {
      ...
      let currentImage = results[0].profile_image; // Current profile image URL
      
      // If no new image is uploaded, keep the existing image URL
      const finalImageUrl = imageUrl || currentImage;

      // Update user profile in the database
      const updateSql = 'UPDATE users SET username=?, email=?, gender=?, age=?, profile_image=? WHERE id=?';
      db.query(updateSql, [username, email, gender, age, finalImageUrl, userId], (updateErr, result) => {
        if (updateErr) {
