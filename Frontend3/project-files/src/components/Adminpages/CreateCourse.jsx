...
const CreateCourses = () => {
  ...
  const handleCourseSubmit = async (e) => {
    ...
    try {
      ...
      setCourseId(response.data.courseId); // Store the course ID from the response
  ...
  const handleLessonSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/lessons', {
        ...lessonFormData,
        course_id: courseId // Use course_id to match the backend requirement
      }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
  ...
  return (
    <div>
      ...
      <div className='create-course-container'>
        ...
        {courseId && (
          <div>
            <h2>{lessonCreated ? "Add More Lessons" : "Create a New Lesson"}</h2>
